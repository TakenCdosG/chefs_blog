...
class Warrior_Twitter extends WP_Widget {
	...
	function Warrior_Twitter() {
		$widget_ops  = array( 'classname' => 'latest-tweets', 'description' => __('Warrior Twitter Tweets Widget', 'familia') );
		$control_ops = array( 'id_base' => 'warrior_twitter' );

		$this->WP_Widget( 'warrior_twitter', __('Warrior Twitter Tweets', 'familia'), $widget_ops, $control_ops );
	}

	function widget( $args, $instance ) {
		extract( $args );

		$warrior_twitter_title        = apply_filters( 'widget_title', empty( $instance['warrior_twitter_title'] ) ? __( 'Latest Tweets', 'familia' ) : $instance['warrior_twitter_title'],  $instance, $this->id_base );
		$warrior_twitter_username     = esc_attr($instance['warrior_twitter_username']);
		$warrior_twitter_tweets_count = ! empty( $instance['warrior_twitter_tweets_count'] ) ? $instance['warrior_twitter_tweets_count'] : 3;
		...
		<div class="inner-widget">
			...
			<?php if ( $tweets ) :
				foreach ( $tweets as $tweet ) : ?>
					<li>
						<p><?php echo twitter_links($tweet['text']); ?></p>
						<div class="entry-meta"><?php echo date('F jS, Y', strtotime($tweet['created_at']) ); ?></div>
					</li>
				<?php endforeach;
			endif; ?>
	        </ul>
...
<?php
	...
	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'warrior_twitter_title' => __( 'Latest Tweets', 'familia' ), 'warrior_twitter_username' => 'themewarrior', 'warrior_twitter_tweets_count' => '3', 'warrior_twitter_button_text' => __( 'Follow me on Twitter', 'familia' ), 'twitter_consumer_key' => '', 'twitter_consumer_secret' => '' ) );
	?>
        <p>
            <label for="<?php echo $this->get_field_id( 'warrior_twitter_title' ); ?>"><?php _e( 'Widget Title:', 'familia' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'warrior_twitter_username' ); ?>"><?php _e( 'Twitter Username:', 'familia' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'warrior_twitter_tweets_count' ); ?>"><?php _e( 'Tweets Count:', 'familia' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'warrior_twitter_button_text' ); ?>"><?php _e( 'Button Text:', 'familia' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_consumer_key' ); ?>"><?php _e( 'Twitter Consumer key:', 'familia' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_consumer_secret' ); ?>"><?php _e( 'Twitter Consumer secret:', 'familia' ); ?></label>
		...
		<p><?php _e( 'You can get Twitter consumer key & consumer secret by <a href="http://dev.twitter.com" target="_blank">creating an app</a> on Twitter.', 'familia' ); ?></p>
	<?php
	}
}
...
function warrior_get_recent_tweets( $screen_name = '', $consumer_key = '', $consumer_secret = '', $tweets_count = 3 ) {
	...
	if ( false === $recent_tweets ) {
		...
		if ( !is_wp_error( $response ) ) {
			...
			echo '<p>'. __( 'Error: Can\'t fetch tweets, your Twitter account probably protected. Please unprotect your account.', 'familia' ) .'</p>';
		}
